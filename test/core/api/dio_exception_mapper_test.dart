...
void main() {
  const DioExceptionMapper mapper = DioExceptionMapper(noInternetMessage: _noInternet);

  DioException dioError({required DioExceptionType type, int? statusCode, dynamic data}) {
  ...
  group('DioExceptionMapper', () {
    ...
    test('maps 301 using the data field', () {
      ...
      expect(
        ...
        isA<ServerException>()
            ...
            .having((ServerException e) => e.statusCode, 'statusCode', StatusCode.movedPermanently),
      );
    });
    ...
    test('maps other HTTP failures to ServerException with status', () {
      ...
      expect(
        exception,
        isA<ServerException>()
            .having((ServerException e) => e.message, 'message', 'Slow down')
            .having((ServerException e) => e.statusCode, 'statusCode', StatusCode.tooManyRequests),
      );
    });
  });
