...
void main() async {
  print(
    '${GenerateConstants.blueColorCode} Fix TextController and FocusNode dispose Starting... ${GenerateConstants.resetColorCode}',
  );
  ...
  if (!directory.existsSync()) {
    ...
    print(
      '${GenerateConstants.redColorCode} Fix TextController and FocusNode dispose Closed. ${GenerateConstants.resetColorCode}',
    );
    return;
  }
  ...
  print(
    '${GenerateConstants.greenColorCode} Fix TextController and FocusNode dispose Finished. ${GenerateConstants.resetColorCode}',
  );
}
...
bool searchInFile(File file) {
  ...
  return fileString.contains('=TextEditingController(') ||
      fileString.contains('=FocusNode(');
}
...
List<String> getTextControllersNames(File file) {
  ...
  for (String item in fileString.split(';')) {
    ...
    if (isItemNotCommented &&
        (item.contains('=TextEditingController(') ||
            item.contains('=FocusNode('))) {
...
void writeDispose(File file, List<String> controllersNames) {
  if (file.readAsStringSync().contains('super.dispose')) {
    final splitItems = file
        .readAsStringSync()
        .replaceAllMapped(RegExp(r'\(\s*\)'), (match) => '()')
        .replaceAllMapped(
          RegExp(r'super.dispose\s+\('),
          (match) => 'super.dispose(',
        )
        .split('super.dispose()');
