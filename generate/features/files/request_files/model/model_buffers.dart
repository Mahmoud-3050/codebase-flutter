...
class ModelRequestBuffers extends BaseRequestBuffers{
  ...
  StringBuffer _generateResponseModel({
    ...
    final StringBuffer buffer = StringBuffer();
    buffer.writeln('class ${responseClassName}Model extends ${responseClassName}Response {');
    ...
    ///-> fromJson
    buffer.writeln('  factory ${responseClassName}Model.fromJson(Map<String, dynamic> json) =>');
    ...
    for(MapEntry<String, dynamic> attribute in attributes.entries){
      ...
      buffer.writeln("        ${attribute.key}: json['${keyNames.snakeCase}'] $stringTermNull,");
    }
    // if(isDataModel){
    //   if(isDataList){
