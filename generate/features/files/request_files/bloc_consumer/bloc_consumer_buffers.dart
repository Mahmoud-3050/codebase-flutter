...
class BlocConsumerRequestBuffers extends BaseRequestBuffers{
  ...
  StringBuffer generateImports({
    ...
    buffer.writeln("import '../controller/$requestNameSnakeCase/${requestNameSnakeCase}_cubit.dart';");
    return buffer;
  }
  ...
  StringBuffer generateBody({
    ...
    buffer.writeln(
        '        if (state is ${responseClassName}ErrorState) {');
    buffer.writeln('          showAppSnackBar(');
    buffer.writeln('            context: context,');
    buffer.writeln('            type: ToastType.error,');
    ...
    if (isDataList) {
      ...
      buffer.writeln('        padding: EdgeInsets.symmetric(horizontal: 16.w, vertical: 16.h),');
      buffer.writeln('        decoration: BoxDecoration(');
      buffer.writeln('          borderRadius: BorderRadius.circular(16.r),');
      buffer.writeln('        ),');
