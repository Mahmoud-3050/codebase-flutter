...
class ValidatorMessages {
  ...
  factory ValidatorMessages.fromJson(
    ...
    final defaults = locale == ValidatorLocale.ar
        ? ValidatorMessages.ar()
        : ValidatorMessages.en();

    return ValidatorMessages(
      required: json['error_field_required']?.toString() ??
          json['required']?.toString() ??
          defaults.required,
      validEmail: json['error_valid_email']?.toString() ??
          json['valid_email']?.toString() ??
          defaults.validEmail,
      validPhone: json['error_valid_phone_number']?.toString() ??
          json['valid_phone']?.toString() ??
          defaults.validPhone,
      validText: json['error_valid_text']?.toString() ??
          json['valid_text']?.toString() ??
          defaults.validText,
      validNumbers: json['error_valid_numbers']?.toString() ??
          json['valid_numbers']?.toString() ??
          defaults.validNumbers,
      validPassword: json['error_valid_password']?.toString() ??
          json['valid_password']?.toString() ??
          defaults.validPassword,
      validPasswordConfirm: json['error_valid_password_confirm']?.toString() ??
          json['valid_password_confirm']?.toString() ??
          defaults.validPasswordConfirm,
      passwordUppercase: json['password_uppercase_requirement']?.toString() ??
          defaults.passwordUppercase,
      passwordLowercase: json['password_lowercase_requirement']?.toString() ??
          defaults.passwordLowercase,
      passwordNumber:
          json['password_number_requirement']?.toString() ?? defaults.passwordNumber,
      passwordSpecialChar: json['password_special_character_requirement']?.toString() ??
          defaults.passwordSpecialChar,
      zeroNotAllowed: json['zero_not_allowed']?.toString() ?? defaults.zeroNotAllowed,
      maximumDecimalPlacesAllowed: json['maximum_decimal_places_allowed']?.toString() ??
          defaults.maximumDecimalPlacesAllowed,
      mustBeDigitsCount:
          json['must_be_digits_count']?.toString() ?? defaults.mustBeDigitsCount,
      maximumDigitsAllowed:
          json['maximum_digits_allowed']?.toString() ?? defaults.maximumDigitsAllowed,
    );
  }
  ...
  factory ValidatorMessages.en({
    ...
    return ValidatorMessages(
      ...
      maximumDecimalPlacesAllowed:
          maximumDecimalPlacesAllowed ?? 'Maximum decimal places allowed exceeded',
  ...
  factory ValidatorMessages.ar({
    ...
    return ValidatorMessages(
      ...
      validPasswordConfirm: validPasswordConfirm ?? 'يرجى تأكيد كلمة المرور بشكل صحيح',
      passwordUppercase:
