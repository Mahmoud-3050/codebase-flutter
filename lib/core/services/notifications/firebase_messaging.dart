...
abstract class AppFirebaseMessaging {
  ...
  static void onMessage(LocalNotificationService service) {
    FirebaseMessaging.onMessage.listen((RemoteMessage event) {
      log(
        '@AppFirebaseMessaging:: FCM onMessage: ${event.notification?.title}',
      );
  ...
  static void onMessageOpenedApp(LocalNotificationService service) {
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage event) {
      log(
        '@AppFirebaseMessaging:: FCM onMessageOpenedApp: ${event.notification?.title}',
      );
      // ServiceLocator.instance<GetNotificationsCubit>().fGetNotifications();
      _showLocalNotification(service, event);
    });
  ...
  static Future<void> _backgroundHandler(RemoteMessage event) async {
    log(
      '@AppFirebaseMessaging:: FCM onBackgroundMessage: Title: ${event.notification?.title}',
    );
  }
  ...
  static void subscribeToTopic(String topic) {
    FirebaseMessaging.instance
        ...
        .then((value) {
          log(
            '@AppFirebaseMessaging:: FCM (Success) Subscribe To Topic[$topic]',
          );
        })
        .catchError((error) {
          log(
            '@AppFirebaseMessaging:: FCM (Error) Subscribe To Topic[$topic], Error: ${error.toString()}',
          );
        });
  }
  ...
  static void unsubscribeFromTopic(String topic) {
    FirebaseMessaging.instance
        ...
        .then((value) {
          log(
            '@AppFirebaseMessaging:: FCM (Success) Unsubscribe From Topic[$topic]',
          );
        })
        .catchError((error) {
          log(
            '@AppFirebaseMessaging:: FCM (Error) Unsubscribe From Topic[$topic], Error: ${error.toString()}',
          );
        });
  }
